...
fn is_meridian_generated(contents: &str) -> bool {
    let head = contents.get(..512).unwrap_or(contents).to_ascii_lowercase();
    head.contains("generated") && head.contains("meridian")
}
...
mod tests {
    ...
    fn generated_header_is_recognised() {
        assert!(is_meridian_generated(
            "# Generated by Meridian (theme_export). Managed file.\n"
        ));
        assert!(is_meridian_generated(
            "/*\n * Meridian GTK3 theme — GENERATED from meridian-tokens ...\n */\n"
        ));
    ...
    fn backup_user_file_is_one_time_and_skips_generated() {
        let dir =
            std::env::temp_dir().join(format!("meridian-theme-export-test-{}", std::process::id()));
        ...
        // Meridian-generated file -> no backup.
