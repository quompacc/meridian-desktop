...
pub mod drm_lease;
// linux-drm-syncobj-v1 relies on DRM_IOCTL_SYNCOBJ_EVENTFD and eventfd(2).
// OpenBSD has neither eventfd(2) nor a native equivalent for this Linux
// protocol. Keep the DRM/KMS backend available, but do not compile or expose
// the protocol on OpenBSD.
#[cfg(all(feature = "backend_drm", not(target_os = "openbsd")))]
pub mod drm_syncobj;
pub mod fifo;
pub mod fixes;
